...
class convert(object): #renamed labelme2coco to convert to bypass the sytem library
...
#                (file_name, extension) = os.path.splitext(res)
    ...
    def annotation(self, points, label, num):
        ...
        #multi_poly = MultiPolygon(points)
...
#         h = annotation['bbox'][3]
    ...
    def mask2box(self, mask):
        ...
        rows = index[:, 0]
        clos = index[:, 1]
        #initialized negative infinity in case of no bounding boxes
        left_top_r = np.min(rows, initial=-np.inf)  # y
        
        left_top_c = np.min(clos, initial=-np.inf)  # x

        right_bottom_r = np.max(rows, initial=-np.inf)
        right_bottom_c = np.max(clos, initial = np.inf)
...
if __name__ == "__main__": #can rename path to the annotation folder you have
